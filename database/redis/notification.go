...
func (connector *DbConnector) fetchNotificationsWithLimitDo(to int64, limit int64) ([]*moira.ScheduledNotification, error) {
	...
	if (len(notifications) == len(notificationsLimited)) {
		// this means that all notifications has same timestamp,
		// we hope that all notifications with same timestamp should feat our memory
		c.Send("UNWATCH")
	...
	// and transaction fail (no notifications was deleted) :(
